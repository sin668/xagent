...
作为第三阶段小范围运行的研发执行者，我希望完成“客户分配、状态流转和合规硬门禁 API”，以便系统可以按 BMAD 已冻结方案推进线索完善、客户晋级、客户承接、清洗治理和 LangGraph 局部试点闭环。
...
**目标：** 实现客户分配、状态流转和 C 级报价/合同前合规阻断。

**Files:**

- Modify: `apps/api/app/api/customers.py`
- Modify: `apps/api/app/services/customer_status.py`
- Test: `apps/api/tests/test_customer_assignment_compliance_guards.py`

**Codex 提示词：**

```text
请执行 P3-E6-S5：客户分配、状态流转和合规硬门禁 API。
...
7. 完成后执行两轮独立评审，并写入 _bmad-output/implementation-artifacts/codex-p3-e6-s5-执行结果.md。
8. 不要执行下一个 Story。
```
...
- 支持分配运营/客服/销售负责人。
- 状态流转符合待分配 -> 待首次触达 -> 已触达待回复等路径。
- C 级报价/合同/付款/物流/清关/交付周期动作前必须合规复核。
- 勿扰客户不能进入触达队列。
