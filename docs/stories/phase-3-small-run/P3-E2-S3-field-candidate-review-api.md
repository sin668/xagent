# Story P3-E2-S3：实现字段级采纳、修改后采纳和拒绝 API
...
作为第三阶段小范围运行的研发执行者，我希望完成“实现字段级采纳、修改后采纳和拒绝 API”，以便系统可以按 BMAD 已冻结方案推进线索完善、客户晋级、客户承接、清洗治理和 LangGraph 局部试点闭环。
...
**目标：** 支持人工逐字段采纳、修改或拒绝 AI/人工补全候选，并保存字段级审计。

**Files:**

- Modify: `apps/api/app/api/lead_enrichment.py`
- Modify: `apps/api/app/services/lead_enrichment.py`
- Test: `apps/api/tests/test_field_candidate_review_api.py`

**Codex 提示词：**

```text
请执行 P3-E2-S3：实现字段级采纳、修改后采纳和拒绝 API。
...
7. 完成后执行两轮独立评审，并写入 _bmad-output/implementation-artifacts/codex-p3-e2-s3-执行结果.md。
8. 不要执行下一个 Story。
```

**验收标准：**

- 支持 accept/reject/update API。
- 采纳必须记录 accepted_by/accepted_at。
- 拒绝必须可记录 rejected_reason。
- 无证据关键字段不能被采纳为晋级关键字段。
