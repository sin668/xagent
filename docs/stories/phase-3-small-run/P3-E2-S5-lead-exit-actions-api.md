...
作为第三阶段小范围运行的研发执行者，我希望完成“实现线索调整等级、Watch/Invalid 和放弃动作 API”，以便系统可以按 BMAD 已冻结方案推进线索完善、客户晋级、客户承接、清洗治理和 LangGraph 局部试点闭环。
...
**目标：** 让无法完善线索能有限保留后退出，避免待补全队列堆积。

**Files:**

- Modify: `apps/api/app/api/staging_leads.py`
- Create/Modify: `apps/api/app/services/staging_lead_actions.py`
- Test: `apps/api/tests/test_staging_lead_exit_actions.py`

**Codex 提示词：**

```text
请执行 P3-E2-S5：实现线索调整等级、Watch/Invalid 和放弃动作 API。
...
7. 完成后执行两轮独立评审，并写入 _bmad-output/implementation-artifacts/codex-p3-e2-s5-执行结果.md。
8. 不要执行下一个 Story。
```

**验收标准：**

- 支持 mark-watch、mark-invalid、abandon、grade update。
- 转 Watch/Invalid/放弃必须记录原因。
