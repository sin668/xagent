# Story P3-E3-S1：实现客户晋级最低准入校验服务

状态：实现完成，真实 PostgreSQL 联调待复跑
Sprint：Sprint 3
优先级：P0
Epic：P3-E3
...
作为第三阶段小范围运行的研发执行者，我希望完成“实现客户晋级最低准入校验服务”，以便系统可以按 BMAD 已冻结方案推进线索完善、客户晋级、客户承接、清洗治理和 LangGraph 局部试点闭环。
...
**目标：** 实现 staging 线索晋级客户前的平衡准入规则和硬门禁校验。

**Files:**

- Create: `apps/api/app/services/customer_promotion.py`
- Create: `apps/api/app/schemas/customer_promotion.py`
- Test: `apps/api/tests/test_customer_promotion_eligibility.py`

**Codex 提示词：**

```text
请执行 P3-E3-S1：实现客户晋级最低准入校验服务。
...
7. 完成后执行两轮独立评审，并写入 _bmad-output/implementation-artifacts/codex-p3-e3-s1-执行结果.md。
8. 不要执行下一个 Story。
```

**验收标准：**

- 准入要求客户名称、国家/城市、至少一个联系方式、来源证据、勿扰校验通过。
- Forbidden 来源不得作为晋级关键来源。
- 无来源证据不得晋级。
- 客户类型/经营信号/意向车型可缺失但显示待补全。
