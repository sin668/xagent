...
作为第三阶段小范围运行的研发执行者，我希望完成“实现清洗建议人工确认/拒绝 API”，以便系统可以按 BMAD 已冻结方案推进线索完善、客户晋级、客户承接、清洗治理和 LangGraph 局部试点闭环。
...
**目标：** 清洗建议必须人工确认后才能执行关键动作。

**Files:**

- Modify: `apps/api/app/api/lead_cleanup.py`
- Modify: `apps/api/app/services/lead_cleanup.py`
- Test: `apps/api/tests/test_lead_cleanup_review_api.py`

**Codex 提示词：**

```text
请执行 P3-E4-S2：实现清洗建议人工确认/拒绝 API。
...
7. 完成后执行两轮独立评审，并写入 _bmad-output/implementation-artifacts/codex-p3-e4-s2-执行结果.md。
8. 不要执行下一个 Story。
```
...
- 支持 approve/reject。
- 恢复 Invalid 必须合规/管理员角色。
- 疑似重复和客户级归并需要管理员确认。
- 所有确认/拒绝记录 reviewer 和 reviewed_at。
