...
作为第三阶段小范围运行的研发执行者，我希望完成“移动端客户工作台页面”，以便系统可以按 BMAD 已冻结方案推进线索完善、客户晋级、客户承接、清洗治理和 LangGraph 局部试点闭环。
...
**目标：** 展示已晋级客户，并按下一步动作优先级排序。

**Files:**

- Create/Modify: `apps/mobile/src/pages/customers/index.vue`
- Create: `apps/mobile/src/services/customers*.js`
- Test: `apps/mobile` 相关测试

**Codex 提示词：**

```text
请执行 P3-E7-S3：移动端客户工作台页面。
...
7. 完成后执行两轮独立评审，并写入 _bmad-output/implementation-artifacts/codex-p3-e7-s3-执行结果.md。
8. 不要执行下一个 Story。
```

**验收标准：**

- 显示客户名称、国家城市、等级、联系方式摘要、意向车型摘要、下一步动作。
- 默认排序符合第三阶段方案。
- 筛选支持今日待跟进、C级待合规、有车型意向、待分配。
- 不展示未晋级 staging 线索。
