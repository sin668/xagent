...
作为第三阶段小范围运行的研发执行者，我希望完成“实现客户意向车型 CRUD API”，以便系统可以按 BMAD 已冻结方案推进线索完善、客户晋级、客户承接、清洗治理和 LangGraph 局部试点闭环。
...
**目标：** 客服/销售可维护客户一对多意向车型。

**Files:**

- Create/Modify: `apps/api/app/api/customer_vehicle_intents.py`
- Create: `apps/api/app/services/customer_vehicle_intents.py`
- Test: `apps/api/tests/test_customer_vehicle_intents_api.py`

**Codex 提示词：**

```text
请执行 P3-E6-S3：实现客户意向车型 CRUD API。
...
7. 完成后执行两轮独立评审，并写入 _bmad-output/implementation-artifacts/codex-p3-e6-s3-执行结果.md。
8. 不要执行下一个 Story。
```
