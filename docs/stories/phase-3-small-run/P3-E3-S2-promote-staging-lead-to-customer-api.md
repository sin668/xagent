# Story P3-E3-S2：实现手动准入并晋级客户 API
...
作为第三阶段小范围运行的研发执行者，我希望完成“实现手动准入并晋级客户 API”，以便系统可以按 BMAD 已冻结方案推进线索完善、客户晋级、客户承接、清洗治理和 LangGraph 局部试点闭环。
...
**目标：** 人工确认后将满足条件的 staging 线索晋级到 `customers`、`lead_sources`、`contact_methods`。

**Files:**

- Modify: `apps/api/app/api/staging_leads.py`
- Modify: `apps/api/app/services/customer_promotion.py`
- Test: `apps/api/tests/test_promote_staging_lead_to_customer_phase3.py`

**Codex 提示词：**

```text
请执行 P3-E3-S2：实现手动准入并晋级客户 API。
...
7. 完成后执行两轮独立评审，并写入 _bmad-output/implementation-artifacts/codex-p3-e3-s2-执行结果.md。
8. 不要执行下一个 Story。
```

**验收标准：**

- `POST /staging-leads/{id}/promote-to-customer` 可晋级客户。
- 写入 customers、lead_sources、contact_methods。
- 保存 accepted_fields_json 或等效审计。
- 重复晋级幂等，不生成重复客户。
