...
作为第三阶段小范围运行的研发执行者，我希望完成“实现人工补录 API”，以便系统可以按 BMAD 已冻结方案推进线索完善、客户晋级、客户承接、清洗治理和 LangGraph 局部试点闭环。
...
**目标：** 允许运营/客服/销售手动补录客户信息、联系方式、经营状况和意向车型候选，并保留来源类型。

**Files:**

- Modify: `apps/api/app/api/lead_enrichment.py`
- Modify: `apps/api/app/services/lead_enrichment.py`
- Test: `apps/api/tests/test_manual_enrichment_api.py`

**Codex 提示词：**

```text
请执行 P3-E2-S4：实现人工补录 API。
...
7. 完成后执行两轮独立评审，并写入 _bmad-output/implementation-artifacts/codex-p3-e2-s4-执行结果.md。
8. 不要执行下一个 Story。
```

**验收标准：**

- `POST /staging-leads/{id}/manual-enrichment` 可创建 manual enrichment result。
- source_type 支持 manual_public_info/manual_customer_reply/manual_business_note/unknown。
- unknown 不得作为晋级关键证据。
- 操作人、时间、备注被记录。
