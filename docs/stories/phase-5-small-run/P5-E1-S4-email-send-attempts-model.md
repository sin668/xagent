...
作为第五阶段小范围运行的研发执行者，我希望新增 `email_send_attempts`，记录每次真实邮件发送、失败、重试、退信和服务商消息 ID，以便推进 Prompt 入库治理、邮件自动回复知识库和 EMAIL_REPLY Agent 的受控闭环。
...
**目标：** 新增 `email_send_attempts`，记录每次真实邮件发送、失败、重试、退信和服务商消息 ID。

**建议文件：**

- apps/api/app/models/*email*
- apps/api/alembic/versions/*
- apps/api/tests/*send_attempt*

**验收标准：**

- 记录 provider、from/to、subject/body 快照、status、attempt_count、provider_message_id、error_message、sent_at。
- 可关联 `email_reply_drafts` 和 `outreach_records`。
- 失败和退信状态可查询、可统计。
