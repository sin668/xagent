...
作为第五阶段小范围运行的研发执行者，我希望在 `apps/agents` 定义 EMAIL_REPLY 请求 envelope、输出 schema 和 schema_version 校验，以便推进 Prompt 入库治理、邮件自动回复知识库和 EMAIL_REPLY Agent 的受控闭环。
...
**目标：** 在 `apps/agents` 定义 EMAIL_REPLY 请求 envelope、输出 schema 和 schema_version 校验。
...
- `schema_version` 固定为 `email-reply-v1`。
- 输出包含语言、建议主题/正文、knowledge_hits、risk_flags、auto_send_allowed、manual_review_required、next_action、audit。
- 无效输出进入 failed，不写业务表。
...
```text
请执行 P5-E7-S1：apps/agents EMAIL_REPLY schema 与 envelope。
目标：在 `apps/agents` 定义 EMAIL_REPLY 请求 envelope、输出 schema 和 schema_version 校验。
