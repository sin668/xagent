...
class Phase4GoNoGoReportService:
    @classmethod
    def decisions(cls) -> dict[str, dict[str, str]]:
        return {
            "deep_enrichment": {
                "name": "Deep Enrichment",
                "decision": "Go：继续小范围 active_run",
                "reason": "第四阶段 active_run 只输出字段候选，仍由 apps/api 人工审核后写入；样本指标显示无证据候选率为 0%。",
                "next_step": "继续小范围运行，重点观察人工接受率、无证据候选和 provider 暂态失败。",
            },
            "lead_cleanup": {
                "name": "Lead Cleanup",
                "decision": "Go：继续小范围 active_run",
                "reason": "第四阶段 active_run 只输出清洗建议，不自动归并、不自动恢复 Invalid；样本指标显示错误合并建议数为 0。",
                "next_step": "继续小范围运行，重点观察重复建议准确率、人工拒绝率和超时案例。",
            },
            "source_discovery": {
                "name": "Source Discovery",
                "decision": "No-Go：保持 shadow_run",
                "reason": "Source Discovery 30 条 shadow 样本存在 Forbidden 误放，来源发现仍不得进入 active_run。",
                "next_step": "补充 Forbidden 来源回归样本，修正过滤和风险阻断规则后再做更大样本 shadow。",
            },
            "lead_extraction_grading": {
                "name": "Lead Extraction/Grading",
                "decision": "No-Go：保持 shadow_run",
                "reason": "Lead Extraction/Grading 30 条 shadow 样本存在硬规则不一致，且联系方式和证据仍有失败案例。",
                "next_step": "强化硬规则优先级、证据命中和联系方式反编造后，再做 shadow 对照复核。",
            },
            "api_retry_worker": {
                "name": "`apps/api` retry worker",
                "decision": "No-Go：下一阶段暂不开始废弃",
                "reason": "Source Discovery 与 Lead Extraction/Grading 尚未具备切换条件，且第四阶段仍需保留 apps/api 临时兼容摘要和既有 retry worker。",
                "next_step": "待核心链路 shadow 阻塞风险解除、active_run 覆盖面扩大并完成独立迁移 Story 后，再规划废弃。",
            },
        }
    ...
    def render_markdown(cls) -> str:
        ...
        for item in decisions.values():
            lines.append(
                f"| {item['name']} | {item['decision']} | {item['reason']} | {item['next_step']} |"
            )
