...
def test_source_discovery_graph_rejects_active_mode_and_private_collection_actions() -> None:
    runner = SourceDiscoveryGraphRunner(search_tool=MockPublicSourceSearchTool())
    state = SourceDiscoveryGraphState(
        discovery_run_id="11111111-1111-1111-1111-111111111111",
        market="Russia",
    ...
    try:
        ...
        assert "Source Discovery 第四阶段只允许 shadow_run" in str(exc)
    else:
        raise AssertionError("Source Discovery graph must reject active mode in phase 4")
