...
class LLMClient:
    ...
    async def generate_json(
        ...
        try:
            if self.http_client is not None:
                response = await self.http_client.post(f"{base_url}/chat/completions", json=payload, headers=headers)
            else:
    ...
    def _api_key_value(self) -> str | None:
        if self.settings.llm_api_key is None:
            return None
        return self.settings.llm_api_key.get_secret_value() or None
