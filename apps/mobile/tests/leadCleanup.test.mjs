...
const rawSuggestion = {
  ...
  evidence_json: {
    lead_name: 'Watch Dealer Moscow',
    evidence_note: '官网联系页公开邮箱；未登录、未互动。',
    evidence_links: ['https://dealer.example.ru/contact'],
    contacts: [
      { type: 'email', value: 'sales@watch-dealer.example.ru', usage: '公开联系邮箱' },
      { type: 'telegram', value: '@watchdealer', usage: '公开社媒账号' },
    ],
...
test('清洗建议 mapper 展示 suggestion_type、原因、证据、目标线索和权限提示', () => {
  ...
  assert.equal(mapped.leadDisplayName, 'Watch Dealer Moscow');
  ...
  assert.deepEqual(mapped.contacts.map((contact) => contact.value), ['sales@watch-dealer.example.ru', '@watchdealer']);
...
test('清洗建议 mapper 兼容 evidence 和根字段里的联系方式', () => {
  const mapped = mapCleanupSuggestion({
    ...rawSuggestion,
    contacts_json: [{ type: 'phone', value: '+7 495 000-00-00' }],
    evidence_json: {
      customer_name: 'Fallback Dealer',
      contact_methods: [{ method_type: 'whatsapp', contact_value: '+7 999 000-00-00' }],
    },
  });

  assert.equal(mapped.leadDisplayName, 'Fallback Dealer');
  assert.deepEqual(
    mapped.contacts.map((contact) => `${contact.type}:${contact.value}`),
    ['whatsapp:+7 999 000-00-00', 'phone:+7 495 000-00-00'],
  );
});

test('清洗建议 mapper 在缺少客户名称时用来源域名作为标题', () => {
  const mapped = mapCleanupSuggestion({
    ...rawSuggestion,
    lead_name: '',
    customer_name: '',
    evidence_json: {
      evidence_links: ['https://www.avtogermes.ru/company/contacts/'],
    },
  });

  assert.equal(mapped.leadDisplayName, 'avtogermes.ru');
});
